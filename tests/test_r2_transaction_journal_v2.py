...
class R2TransactionJournalV2Tests(unittest.TestCase):
    ...
    def test_fresh_process_reconstructs_one_authoritative_chain(self):
        ...
        self.assertEqual(len(restarted.durable_authority_claims), 2)
        self.assertEqual(restarted.current_head_fingerprint, journal.current_head_fingerprint)
        self.assertEqual(restarted.next_legal_action, "CLAIM_FRESH_AUTHORITY_OR_TERMINAL")
    ...
    def test_every_journal_cut_point_reconstructs_and_reports_next_action(self):
        ...
        states = [(journal, "CLAIM_FRESH_AUTHORITY")]
        journal = journal.append_authority_claim(
        ...
        states.append((journal, "CLAIM_FRESH_AUTHORITY_OR_TERMINAL"))
    ...
    def test_torn_unknown_duplicate_predecessor_owner_and_replay_fail_closed(self):
        ...
        replay = _claim(
            self.binding,
            fresh.current_head_fingerprint,
            authority_fingerprint=self.genesis.authority_claim.authority_fingerprint,
        )
        with self.assertRaisesRegex(JournalV2Error, "R2_JOURNAL_V2_INVALID"):
            fresh.append_authority_claim(
                claim=replay,
                transition_instance_fingerprint=TRANSITION,
            )
...
def _genesis(binding):
    claim = DurableAuthorityClaimV2.create(
        ...
        prior_journal_head_fingerprint=PRE_HEAD,
        claim_sequence=1,
...
def _claim(binding, head, *, authority_fingerprint="f" * 64):
