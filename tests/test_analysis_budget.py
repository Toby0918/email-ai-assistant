...
class AnalysisBudgetTests(unittest.TestCase):
    def test_start_uses_one_13_second_monotonic_budget(self) -> None:
        now = [100.0]

        budget = AnalysisBudget.start(clock=lambda: now[0])

        self.assertEqual(budget.deadline, 113.0)
        self.assertEqual(budget.remaining_seconds(), 13.0)
        self.assertEqual(budget.remaining_seconds(reserve_seconds=2.0), 11.0)
