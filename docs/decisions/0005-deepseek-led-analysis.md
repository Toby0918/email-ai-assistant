...
- The backend calls `AnalysisBudget.start()` immediately before reading and validating the request body; the runtime order is `start -> read -> api`, so body-read time consumes the cooperative 13-second monotonic target. The same target has an 8-second hard parser/OCR process deadline, a DeepSeek maximum/default of 10 seconds, the pre-existing general/Ollama maximum of 25 seconds, a provider minimum of 5 seconds, and a fixed 2-second validation/response reserve. The shared target therefore limits a fresh Ollama attempt to 11 seconds even though its general cap remains 25 seconds.
