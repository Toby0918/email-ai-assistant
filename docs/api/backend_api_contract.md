...
- 通用附件文本清洗不对“RFQ”等标签做长数字豁免，并必须删除嵌入字母令牌中的 7 位及以上数字串。专用组件提取器只能在严格标签/值语法下构造商业编号；纯数字值限 4-9 位，任何值最多只能包含 9 个数字，而且必须在剔除唯一一个 RFQ/PO/INV 等前缀后再拒绝重复前缀、电话、卡号/账号、路径和 URI/域名形状。
- 请求动作只保留固定动词和对象类别，质量问题只保留固定信号标签，截止时间只保留明确 cue 和日期/相对时间。每个构造事实必须再通过精确 schema 清洗后才能进入结果、prompt 和 SQLite。
- 截止时间、请求动作和质量信号在构造前必须检查同一有界句段的否定或已解决语义；`not`、`never`、`do not`、`no evidence of` 或 `resolved/fixed/closed` 等上下文不得生成正向事实。
