...
[ ] Issue #55 ACL capture is fixed-role; parent and finance are compare-only, source compatibility is complete/read-only, and only the newly created empty Container receives the protected three-principal inheritable DACL.
...
[ ] Issue #55 no-replace primitives bind opened source/parent handles, 128-bit file ID, fixed NTFS volume, parent identity, reparse-free state, target absence, and same-identity publication.
