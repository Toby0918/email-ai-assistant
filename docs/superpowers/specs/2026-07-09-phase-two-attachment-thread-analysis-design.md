...
The generic attachment-text sanitizer continues to redact every seven-or-more-digit number, including contiguous or separator-grouped digit runs embedded in alphabetic tokens, together with emails, phones, card/account-like values, paths, and URLs. A separate bounded component extractor may construct a business identifier only from strict label/value syntax. Pure numeric identifiers require an explicit label and 4-9 digits; every identifier contains at most nine digits, uses no dot, slash, backslash, at-sign, or colon, and may contain only one recognized RFQ/PO/INV-style prefix. Validation rejects repeated wrappers, phone and account shapes, paths, and URI/domain values. Deadline, requested-action, and quality facts are also rejected when their bounded local clause is negated, reversed, or already resolved. Every constructed fact passes a final exact-schema sanitizer before it may enter results, model context, or SQLite.
