...
The preferred transport is a bounded byte upload to the local backend, because Tencent Exmail downloads can require the browser's authenticated session. The extension must reject unsupported resource types before transfer and show a per-resource limitation when extraction is unavailable. When an announced `Content-Length` already exceeds a byte limit, response-body cancellation is best effort and must never delay the immediate `frontend_limit` result, even if the browser cancellation promise does not settle. This is a controlled fallback for current-message resources, not a general browser download service.

No authorized production Tencent Exmail attachment DOM snapshot or verified attachment-container class/id is stored in this repository. The current fail-closed mapping therefore uses only structural evidence in the top-level document: one globally unique visible strict subject element, one globally unique visible external From/To header region, and one globally unique selected known body root must produce a unique non-`BODY`/non-`HTML` envelope. That envelope must be a direct child of `document.body`, and the selected body root must be a direct child of the envelope; a child-frame message, unknown outer wrapper, unknown intermediate wrapper, or additional visible known body root disables resource collection with one `resource_unavailable` limitation while body analysis continues. Resource controls are considered only in direct sibling subtrees outside the sole known body root and must be visible `a[href]` or `img[src]` controls whose URL is same-origin HTTPS with a non-empty query on `/cgi-bin/download` or `/cgi-bin/viewfile`. This mapping is covered by sanitized synthetic fixtures only; authorized real Tencent Exmail smoke validation remains pending and is not claimed.
...
The generic attachment-text sanitizer redacts every sequence of seven or more digits, including ISO dates and runs joined across whitespace or common separators inside alphabetic tokens, together with emails, paths, and URLs. A separate bounded component extractor may construct a business identifier only by full-matching a complete labeled field. Identifier values use only `[A-Z0-9_-]`; pure numeric values require 4-9 digits, every identifier contains at most nine digits, and at most one recognized prefix must agree with the field label. Repeated or heterogeneous wrappers, grouped phone/account shapes, paths, and URI/domain values fail closed. A table identifier requires exactly two non-empty label/value cells; any continuation cell invalidates the whole row identifier. Deadline, requested-action, and quality candidates are evaluated within comma/semicolon/conjunction-bounded clauses using finite tokens before and after the candidate. Requested-action verbs pair only with later objects in their own source-ordered clause. An affirmative verb may carry across `but/however` into the immediately following object-only clause only when it has not already paired an object; constructing a fact clears that pending verb. Negated or cancelled clauses never delete a later valid action. Adjacent `0/zero/nil/non-` and `-free/ free` quality forms are absent, and resolution terms retire a quality fact only when they are not locally negated. Deadline forms stating `not required`, `does not apply`, `no longer applicable`, or `optional` are absent requirements. Every constructed fact passes a final exact-schema sanitizer before it may enter results, model context, or SQLite.
