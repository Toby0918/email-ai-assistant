...
all providers disabled by default：`EMAIL_AGENT_LLM_PROVIDER=disabled`、`EMAIL_AGENT_TEXT_FALLBACK_PROVIDER=disabled`，DeepSeek 的 `EMAIL_AGENT_DEEPSEEK_OUTPUT_MODE` 默认也是 `conservative`。只有显式后端配置才能启用 OpenAI 主路线、DeepSeek text fallback 或独立 DeepSeek-led 路线。

Option C 的最大调用链为 one OpenAI multimodal primary call、one DeepSeek text-only fallback、deterministic rules last。每个 provider 都只调用一次且 `max_retries=0`。OpenAI 主路线固定 `https://api.openai.com/v1`、模型 `gpt-5.6-sol` 和 Responses API。OpenAI omits `text.format`; the JSON-only prompt is enforced by strict local validation. 请求保留 `store=false`、`stream=false`、no tools（`tools=[]`）、`text.verbosity=low`、`reasoning.effort=low`、图片和文件 `detail=high`，以及 `max_output_tokens=2400`（2,400）；返回内容必须通过拒绝 duplicate key 的本地 JSON 解码和严格的 envelope/schema/evidence/privacy/grounding/safety 校验。DeepSeek 路线继续使用固定 system message 与 `response_format={"type": "json_object"}`，并通过 `extra_body={"thinking": {"type": "disabled"}}` 明确关闭 thinking。所有 Prompt 只允许返回 JSON，不提供工具或邮箱动作能力。

OpenAI 只接收 selected locally screened current-message media 与当前可见、去标识文本。DeepSeek text fallback 永不接收媒体或二进制，只在 eligible OpenAI provider failure、显式启用 fallback 且最终预算采样仍有至少 12 秒时调用；privacy、private-artifact、routing 或 budget block 不进入 fallback。
...
模型不是直接返回公开 API 对象。OpenAI 与 DeepSeek 都必须返回同一个内部 `deepseek_analysis_v1` envelope，其中包括 request-local source ID、`field_evidence`、分析字段和附件增强。后端先校验 JSON、内部 schema、语言、来源成员关系、关键事实 grounding 和安全边界，再映射到不变的公开分析 schema。
...
在模型路线中，user message 是一个有界 JSON context object，包含 `context_type=current_visible_email`、不可信元数据、后端时间线骨架和带 request-local ID 的 `sources`。可见 URL 只替换为 `[link present]`；附件使用经清洗、按附件与请求总量截断的 ephemeral sanitized text。OpenAI 可额外接收筛选后媒体，DeepSeek 不接收媒体。序列化前必须从元数据、线程和附件文本统一删除使用冒号、等号、copula、whitespace-only separator 或引号连接的 credential/password/API key/session ID 值，同时保留 password-reset status、API-key rotation policy、token expiry、cookie policy 和 session-ID expiry 等无密钥业务说明。该扩展上下文不得进入公开响应、SQLite 或日志。
...
浏览器在独立 20-second resource collection 后使用 60-second POST wait。后端请求共享一个 55-second cooperative target；parser/OCR 使用 hard 8-second deadline，OpenAI 最多 35-second，DeepSeek 最多 10-second。只有剩余至少 12-second 时才可启动 text fallback，并始终保留 5-second response/persistence reserve。

`attachments` 只包含当前邮件页面已显示的附件元数据，不构成附件事实。text/hybrid 来源只有在 evidence 与实际发送的 source 文本相符时才能支撑字段。visual-only 内部标记在 OpenAI 出站投影前转换为固定、已去标识的自然语言来源描述，且不进入 DeepSeek；若允许列表中的观察确实可见，OpenAI 必须生成一个 source-bound `attachment_augmentations` item，并为每个 leaf 提供 evidence。visual-only 仍只允许输出受限的完整定性句并增强其 matching attachment insight；不得支撑 global fields、identity、protected traits、precise facts、commands、commitments 或 outcomes，也不得改变 attachment status/limitations。`metadata_only`、`unavailable` 或 `failed` 项只能用于说明 `limitations`、缺失信息和人工核查要求，不能推断价格、数量、交期、合同或质量结论。

跨语言支持只允许 body-only fixed cross-language bridge：仅固定模板的 `summary` 与 `priority_reason` 可引用实际发送的正文投影。它不得桥接附件视觉、未发送历史、参与者身份或任何精确事实。

## 输出要求

规则路线输出公开 schema JSON；所有模型路线输出 `deepseek_analysis_v1` JSON。它们最终都遵循 `docs/data/analysis_result_schema.md` 的公开结构。无法判断时使用 `unknown`、空数组或简短说明。
...
- `conversation_timeline` 和附件骨架继续由后端确定性生成；模型只能为已有 source 提供 grounded wording。文本增强只用于 `parsed` 附件；视觉定性增强只用于 matching attachment insight 并可保持 `metadata_only`。模型不能改写附件状态、删除开放项或新增来源。
