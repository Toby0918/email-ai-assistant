...
GUARDRAILS = [
    ...
    (
        "R2 historical closure evidence rollover boundary",
        "docs/operations/r2_closure_evidence_rollover_task_brief.md",
    ),
    (
        "R2 GitHub unattributed-approval compatibility",
        "docs/operations/r2_github_guardrail_unattributed_approval_compatibility_task_brief.md",
    ),
    (
...
Stable maintenance observation deepening is implemented locally in `scripts/maintenance_scan.py` and the private closure adapter. The parameterless fresh seam owns fixed scanner composition, observer-owned immutable stable records, deterministic ordering, duplicate rejection and severity counts with no cache, persistence, closure fingerprint, caller scanner or callback. `local_evidence.py` remains the sole closure consumer and retains the independent exact twenty-four-entry registry, canonical fingerprint and eligibility decision. The explicit materialized-tree seam is internal and accepts only a root plus a sorted unique safe tracked-path tuple. Existing maintenance CLI arguments, rendered `message`/`fix`, output and exit codes remain unchanged. This worktree-only refactor does not roll over or recreate closure evidence, approve Issue #38, authorize or execute Issue #39, commit, push, open a PR, or merge.

Historical closure evidence rollover is implemented in the independent five-file `backend.r2_closure_evidence_rollover` package and fixed `scripts/rollover_r2_solo_maintainer_closure.py run` entry. Its parameterless coordinator creates one canonical 300-second single-use candidate bound to a clean exact current master, a strict historical-ancestor closure, exact manifest/receipt bytes and candidate linkage, Windows file/stream/DACL identities, separately bound Git-common parent identity/DACL, and one deterministic absent historical target. Wall and private monotonic clocks enforce the half-open window at entry and commit. Execution rederives the complete state before the native commit boundary, reads bytes through writer-excluding handles, and supports a production parent without `FILE_DELETE_CHILD` through one fixed owner-delete source-DACL bridge under a pending candidate-bound parent namespace guard. It obtains the rename handle, restores and verifies the exact protected source DACL, closes the temporary control/parent handles, retains a pending source-directory oplock across the child-handle release required by Windows, verifies the candidate-bound parent handle again, and performs only a same-parent, same-volume, no-replace directory rename while preserving bytes, identities, and final DACL. It never changes the Git-common parent DACL and has no caller path/ref/repository/command, copy, delete, overwrite, repair, cleanup, pathname rollback, Issue #38 approval, execution authority, or Issue #39 authority. Automated verification uses only test-owned temporary evidence and never reads, changes the DACL of, or moves the real Git-common closure. The failed live rollover on `3ca0faddb9b6384941974c23e891aac18588246b` stopped at the pre-rename source-directory `DELETE` open; read-only disposition left the active source, absent historical target, protected DACL and artifact hashes unchanged. This maintenance fix did not retry live rollover.

Issue #39 one-command Project Container orchestration is implemented in `backend.r2_issue39_orchestrator` and `scripts/execute_project_container_cutover.py`. Its governed code allowlist permits only that fixed composition root, the fixed script, and the package-owned retained restart runner; the three historical standalone preflight/evidence/transaction roots remain `DORMANT_NO_ISSUE39_APPROVAL`. The initial wrapper accepts only the code-fixed registered `issue39-governed-enablement` launcher worktree and rejects legacy, alternate, copied, or reparse launchers before importing the orchestrator. Its production order is zero-mutation closure/Issue #38/input/complete dynamic-roster readiness, fixed real-console confirmation, exact incident disposition, fresh complete prepare, create-only evidence plus retained-anchor transfer, and a dynamic closed catalog. Archive-parent readiness binds the exact `D:\\IncidentArchives\\email_ai_assistant\\issue38` hierarchy state, component presence, and opened identities as `PROVISIONABLE`, `READY`, or `BLOCKED` into the separate incident confirmation. After confirmation, disposition must reproduce that exact fingerprint before create, then hold and placement-revalidate the validated/created chain through the no-replace rename and artifact reread. Native parent-relative `FILE_CREATE`, fixed-drive NTFS, non-reparse exact placement, and the protected current-operator/LocalSystem/Administrators inheritable Full Control DACL remain mandatory; drift, post-confirmation competition, parent replacement, and collision stop before the source move, partial state is retained, and the drive-root DACL is unchanged. Fresh repository preparation requires filter-free HEAD-tree/index equality, ordinary index flags, an empty untracked set, and stable regular stage-zero index/config/source-absence evidence before and after review. Its manifest binds each index OID separately from the exact raw relocation size/SHA-256 and permits only direct raw blob equality or the code-owned no-NUL/no-bare-CR CRLF-to-LF projection with exact index-OID equality. Projection requires include-free exact true repository/worktree `core.autocrlf`, or the fixed Git system true mode when no override exists. Tracked `.gitattributes`, fixed `.git/info/attributes`, and effective repository/system `core.attributesFile` are rejected; no `check-attr`, Git filter, attribute file, or encoding driver runs. The portable six-worktree synthetic baseline remains 27 actions/24 host effects; a 2026-08-29 read-only live observation found 14 linked worktrees, which would derive 35/32 if unchanged, but fresh prepare is authoritative. Every preflight, evidence/bootstrap, catalog, recovery and terminal V3 confirmation first displays one strict content-free bound action-context line, followed by the fresh candidate and fixed acknowledgement. Every host effect uses a fresh action-bound durable claim and journaled intent; restart classifies two stable observations without replaying an already-present effect, while rollback retains failed state. Terminal success requires a journal-reconstructed ordered validation receipt and two fresh full audits of layout, roster, Git, ACL, managed units, provider-disabled service identity, and the single deterministic rule-fallback row. The authorized real command on `d5063fd2d86821c2f927ae745a60682b79c7c9b8` archived the fixed incident evidence and durably completed the three before-evidence preflight observations, then stopped in `prepare_evidence` with `INCIDENT_STOP`, `accepted=0`, and `host_actions=0` when the clean legacy Windows checkout's CRLF bytes differed from LF index blobs. This forward repair has not retried evidence publication, cutover, resume, or rollback. Code enablement, tests, CI, and merge are implementation evidence only; closure remains a separate governed ceremony, and real execution still requires a separate final authorization.
