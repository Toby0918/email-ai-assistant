"""Whole-tree ownership for the fixed verifier worker process."""
...
from .canonical import VerifierProcessError
...
class ProcessTree:
    """Own one POSIX process group or Windows kill-on-close Job."""
    ...
    def attach(self, process: subprocess.Popen) -> None:
        if os.name == "nt":
            try:
                ...
                resumed = (
                    attached and _resume_windows_process(process)
                )
            except Exception:
                resumed = False
            if not resumed:
    ...
    def finish(self, process: subprocess.Popen) -> int:
        if os.name != "nt":
            ...
            with self._lock:
                if self._posix_open:
                    group = self._process_group or process.pid
                    self._process_group = None
                    self._posix_open = False
                    _kill_posix_group(group)
        return process.wait()
...
def _assign_windows_job(
    ...
    if handle is None or process_handle is None:
        return False
    return bool(
