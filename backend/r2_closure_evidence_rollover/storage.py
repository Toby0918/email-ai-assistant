...
from backend.r2_solo_maintainer_closure.storage import (
    ...
    _git_common_dir,
    _identity,
    _prepare_windows_terminal,
    _publication_conflict_names,
    _read_locked_acl,
    _release_file_guards,
    _request_oplock,
    _require_exact,
    _safe_directory,
    _settle_oplocks,
...
from .repository import (
    ClosureEvidenceObservationV1,
    create_evidence_observation as _create_observation,
...
class FixedClosureEvidenceStorage:
    ...
    def collect(self) -> ClosureEvidenceObservationV1:
        try:
            ...
            target_name = _target_name(
                manifest.final_commit_oid, manifest.manifest_fingerprint
            )
            ...
            return _create_observation(
                ...
                historical_target_name=target_name,
                source=source,
            )
    ...
    def commit(self, observation: ClosureEvidenceObservationV1, before_commit) -> None:
        if (
            type(observation) is not ClosureEvidenceObservationV1
            or observation.source is None
        ...
        try:
            ...
            _guarded_rollover(
                ...
                observation.parent_dacl_sha256,
                before_commit,
            )
            ...
            if (
                after != observation.evidence_identity_fingerprint
...
def _guarded_rollover(
    ...
    if (
        ...
        or _identity(os.lstat(source)) != identity
    ):
        raise ClosureEvidenceRolloverError(RolloverErrorCode.PUBLICATION_REJECTED)
    rename, pointer, size, close = _prepare_windows_terminal(target)
    guards = []
    try:
        _open_rollover_guards(source, payloads, identity, guards, close)
        ...
        _require_source_guard(guards[-1], expected_acl[-1], pending=True)
        _require_parent_guard(
            guards[0], parent_identity, parent_dacl, source.name, target.name
        )
...
def _open_rollover_guards(source, payloads, identity, guards, close) -> None:
    for name, payload in zip(_FILES, payloads, strict=True):
        ...
        if _windows_streams(handle) != (("::$DATA", len(payload)),):
            raise ClosureEvidenceRolloverError(RolloverErrorCode.PUBLICATION_REJECTED)
    directory = _windows_open(source, 0x00030080, 0x5, 0x42200000)
    guards.append((directory, *_request_oplock(directory, 1, close), identity))

def _require_rollover_guards(guards, payloads, expected_acl, pending: bool) -> None:
    if (
        _windows_streams(guards[-1][0]) != ()
    ...
    for guard, payload in zip(guards[:-1], payloads, strict=True):
        if (
            _windows_streams(guard[0]) != (("::$DATA", len(payload)),)
...
def _require_source_guard(guard, expected_acl: bytes, *, pending: bool) -> None:
    if (
        pending and _wait(guard[1], 0) != _WAIT_TIMEOUT
...
def _require_parent_names(handle, source_name: str, target_name: str) -> None:
    ...
    if (
        folded.count(source_name.casefold()) != 1
