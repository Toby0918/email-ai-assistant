...
def initialize_private_keys(
    ...
    *,
    ...
    authority_payload = _AUTHORITY_MAGIC + _random(rng, 96)
    candidate_payload = _CANDIDATE_MAGIC + _random(rng, 32)
    try:
        _write_envelope(authority / _AUTHORITY_FILE, "authority", protector.protect(authority_payload))
        _write_envelope(candidate / _CANDIDATE_FILE, "candidate", protector.protect(candidate_payload))
