...
第一阶段只做“点击按钮分析当前邮件”。第二阶段允许在用户点击后，从当前打开邮件页面中传输可见的受支持附件和会话上下文到本地后端进行受限分析；系统不自动遍历邮箱、不自动分析所有邮件、不自动发送邮件、不删除邮件、不归档邮件，也不读取其他邮件、文件夹或账户数据。
...
- 自动扫描邮箱或自动分析所有邮件。
- 未经用户点击或明确授权就分析邮件内容。
- 接入真实邮箱账号或读取真实邮箱数据，除非后续单独确认。
- 在前端保存、硬编码或暴露 OpenAI API key。
- 自动代表用户承诺价格、交期、付款、合同或法律事项。
- 在用户点击前收集附件、会话或邮件数据，或读取其他邮件、文件夹或账户数据。

## 技术栈基线

Python 后端负责邮件清洗、AI 调用、JSON 校验、SQLite 持久化和本地 API。AI 调用可以是后端 OpenAI，或在用户单独确认后启用的后端本地 Ollama/Qwen；默认必须保持关闭或规则兜底。必须保留现有版本约束：

- Python 3.12.13，不可使用更高版本。
- SQLite 3.50.4，不可使用更高版本。
- beautifulsoup4 4.15.0，不可使用更高版本。
- openpyxl 3.1.5，不可使用更高版本。
- openai 2.44.0，不可使用更高版本。
- python-dotenv 1.2.2，不可使用更高版本。
- pypdf 5.7.0、python-docx 1.1.2、Pillow 11.2.1、pytesseract 0.3.13，仅用于后端受限附件解析；Tesseract 可执行程序缺失时只能降级 OCR，不得阻断规则兜底。

辅助窗口前端负责识别当前邮件、提供“分析此邮件”按钮、在点击路径中收集当前邮件可见的受支持资源、调用后端 API、展示分析结果。前端路线为 Chrome / Edge 浏览器扩展和本地调试页面；前端不允许保存 API key，也不允许直接调用 OpenAI API、Ollama API、Qwen、Gemma 或任何本地模型端点。

## 安全红线

1. 不自动发送邮件、不删除邮件、不归档邮件。
2. 不自动分析所有邮件；附件传输、解析、OCR 和模型推理只能由“分析此邮件”的用户点击触发，且只限当前打开邮件的可见资源。
3. 不读取其他邮件、文件夹或账户数据，不接入 OAuth 或邮箱 SDK。
4. OpenAI API key、Ollama/Qwen 配置、邮箱凭据、OAuth token 和服务端密钥只能放在后端受控环境中。
