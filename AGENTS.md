...
Issue #55 adds only `backend.cutover_host_mutation` fixed-role Windows ACL and
...
returns no mutation capability even for otherwise valid execution
authorization. No real project ACL, repository/worktree, service, Runtime,
SQLite, provider, mailbox, vault, private store, or private data may be touched.
Issues #56 through #59, Issues #38/#39, and parent Spec #50 remain separate.
...
The narrow wire compatibilities accept
`pull_request.parameters.required_reviewers` only when absent or exactly `[]`;
`pull_request.parameters.require_extra_approval_for_unattributed_changes` may
be absent or exactly `true` only when `required_approving_review_count` is the
exact integer `0`. Those accepted wire-only values are removed before
comparison with the unchanged 965-byte canonical configuration and its
