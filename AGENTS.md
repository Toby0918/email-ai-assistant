...
第一阶段只做“点击按钮分析当前邮件”。系统不自动遍历邮箱、不自动分析所有邮件、不自动发送邮件、不删除邮件、不归档邮件，也不接入真实邮箱账号，除非后续单独确认。
...
辅助窗口前端负责识别当前邮件、提供“分析此邮件”按钮、调用后端 API、展示分析结果。前端路线待选：Outlook Add-in、Google Workspace Add-on、Chrome / Edge 浏览器扩展或本地调试页面。前端不允许保存 API key，也不允许直接调用 OpenAI API、Ollama API 或任何本地模型端点。

## 安全红线

1. 第一阶段不自动发送邮件、不删除邮件、不归档邮件。
2. 第一阶段不自动分析所有邮件，只允许先做“用户点击按钮后分析当前邮件”。
3. 不允许接入真实邮箱账号或读取真实邮箱数据，除非后续单独确认。
