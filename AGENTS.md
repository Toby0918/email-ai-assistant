...
Python 后端负责邮件清洗、AI 调用、JSON 校验、SQLite 持久化和本地 API。必须保留现有版本约束：
...
辅助窗口前端负责识别当前邮件、提供“分析此邮件”按钮、调用后端 API、展示分析结果。前端路线待选：Outlook Add-in、Google Workspace Add-on、Chrome / Edge 浏览器扩展或本地调试页面。前端不允许保存 API key，也不允许直接调用 OpenAI API。
...
4. OpenAI API key、邮箱凭据、OAuth token 和服务端密钥只能放在后端受控环境中。
